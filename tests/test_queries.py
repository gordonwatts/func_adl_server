...
from tests.datasets_for_tests import fs_local_test_file, fs_small_rucio_ds, fs_small_rucio_ds_cached
...
@certs_available
def test_query_new_dataset_localds(single_use_auth_cluster_restart):
...
@certs_available
def test_query_new_dataset_localds_same_column(single_use_auth_cluster):
...
@certs_available
def test_query_new_dataset_localds_new_column(single_use_auth_cluster):
    'Run on an existing dataset'
    r = EventDataset(fs_small_rucio_ds) \
        .SelectMany('lambda e: e.Jets("AntiKt4EMTopoJets")') \
...
@certs_available
def test_query_new_dataset_localds_identical_query(single_use_auth_cluster):
    'Run on an existing dataset'
    r = EventDataset(fs_small_rucio_ds) \
        .SelectMany('lambda e: e.Jets("AntiKt4EMTopoJets")') \
        .Select('lambda j: j.eta()') \
...
@certs_available
def test_query_new_dataset_cacheds(single_use_auth_cluster_restart):
    'Run on an new dataset'
...
@certs_available
def test_query_new_dataset_cacheds_same_column(single_use_auth_cluster):
    'Run on an existing dataset'
    r = EventDataset(fs_small_rucio_ds_cached) \
        .SelectMany('lambda e: e.Jets("AntiKt4EMTopoJets")') \
        .Select('lambda j: j.pt()') \
...
@certs_available
def test_query_new_dataset_cacheds_new_column(single_use_auth_cluster):
    'Run on an existing dataset'
    r = EventDataset(fs_small_rucio_ds_cached) \
        .SelectMany('lambda e: e.Jets("AntiKt4EMTopoJets")') \
        .Select('lambda j: j.eta()') \
...
@certs_available
def test_query_new_dataset_cacheds_identical_query(single_use_auth_cluster):
    'Run on an existing dataset'
    r = EventDataset(fs_small_rucio_ds_cached) \
        .SelectMany('lambda e: e.Jets("AntiKt4EMTopoJets")') \
        .Select('lambda j: j.eta()') \
